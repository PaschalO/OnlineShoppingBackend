...
export class ProductStore {
	...
	async getProductByCategory(category: string): Promise<Product[] | null> {
		try {
			...
			throw new Error(
				`Could not get products by the given category ${category}: ${error}`
			);
		}
	}
	...
	async getTopFivePopularProducts(): Promise<Product[] | null> {
		try {
			...
			throw new Error(
				`Could not the top five popular products: ${error}`
			);
		}
	}
	...
	async createProduct(product: Product): Promise<Product | null> {
		try {
			const connection = await Client.connect();
			const sql =
				"INSERT INTO products (name, price, category, description, image, in_stock, created_at, modified_at) VALUES($1, $2, $3, $4, $5, $6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING *";
			const result = await connection.query(sql, [
				product.name,
				product.price,
				product.category,
