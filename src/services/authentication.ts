    ...
    try {
        ...
        if (user) {
            if (await bcrypt.compare(password + PEPPER, user.password)) {
                ...
                const token = await jwt.sign({...user, password: ''}, TOKEN_SECRET, jwtOptions);
                res.status(200).json(token);
                return;
            }
            res.status(400).json({message: 'The password or email you entered is incorrect'});
            return;
        }
        res.status(400).json({message: 'The credentials you entered isn\'t connected to an account. Find your account and log in'});
    }

    catch (error) {
        res.status(400).json({message: ` Authenticating user failed ${error}`});
    }
}
